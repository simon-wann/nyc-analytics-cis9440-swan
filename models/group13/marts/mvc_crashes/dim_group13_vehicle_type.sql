    ...
    SELECT
        *,
        ...
        CASE
            ...
            WHEN LOWER(vehicle_type_code1) LIKE '%van%' THEN 'Van / Utility Vehicle'
            WHEN LOWER(vehicle_type_code1) LIKE '%pick-up%' THEN 'Truck'
            WHEN LOWER(vehicle_type_code1) LIKE '%box truck%' THEN 'Truck'
            WHEN LOWER(vehicle_type_code1) LIKE '%tractor truck%' THEN 'Truck'
            WHEN LOWER(vehicle_type_code1) LIKE '%bus%' THEN 'Bus / Transit Vehicle'
            WHEN LOWER(vehicle_type_code1) LIKE '%bike%' THEN 'Bike / Micromobility'
